...
# =============================================================================
# [Agent State] 日报状态生成与保存
# =============================================================================
def build_state_note_title(project_name, target_date_str, project_guid=""):
    """
    生成 daily_state 笔记标题。

    示例：
    2026-04-27_算法部_daily_state_a13f92bc
    """
...
def flatten_parsed_result(parsed_result, note_entry, department_name="", target_date_str=""):
    """
    将 DailyReportParser 的结构：

    member -> projects -> sections

    转成适合日报/周报 agent 使用的 normalized_items。
    """
    items = []
            ...
            for section_key in ("progress", "issue_help", "next_focus"):
                ...
                for idx, block in enumerate(section_items):
                    ...
                    items.append({
                        ...
                        "source_url": f"{BASE_URL}/workspace/{note_guid}" if note_guid else "",
                        "member": {
                            "label": member_label,
                            "uid": member_uid,
                            "id": member_id,
...
def group_items_by_project(items):
    """
    将 normalized_items 按项目聚合。
    """
    grouped = OrderedDict()
...
def build_project_timelines(items):
    """
    将 normalized_items 按 project -> date -> section 组织，
    作为后续周报追踪的基础。
    """
...
def build_daily_agent_state(project, target_date_str, parsed_note_entries):
    """
    生成日报 agent state。
    """
    normalized_items = []
            ...
            "note_entries": [
                {
                    ...
                    "source_url": f"{BASE_URL}/workspace/{x.get('note_guid')}"
                }
                for x in parsed_note_entries
            ]
        },
        "normalized_items": normalized_items,
        "grouped_by_project": group_items_by_project(normalized_items),
        "project_timelines": build_project_timelines(normalized_items),
        "check_results": {
            "parse_summary": {
...
def save_daily_state_note(project, daily_state):
    """
    将 daily_state 保存成平台笔记。

    保存位置来自 config：
    - state_target_project_guid
    - state_target_parent_guid
    - state_target_user_guid 可选，不配则使用全局 USER_GUID
    """
    ...
    # 默认使用全局 USER_GUID 创建 state
    state_target_user_guid = project.get("state_target_user_guid") or USER_GUID
    ...
    state_url = f"{BASE_URL}/workspace/{doc_id}" if doc_id else ""
...
def step1_summary_note(project):
    ...
    try:
        ...
        daily_state = build_daily_agent_state(
            project=project,
            target_date_str=target_date_str,
            parsed_note_entries=parsed_note_entries
        )

        state_publish_result = save_daily_state_note(project, daily_state)
...
# =============================================================================
def insert_markdown_to_note(user_guid, note_guid, markdown_content, convert_special=True):
    """
    写入 Markdown 到笔记。

    convert_special=True:
        普通日报正文使用，会把 mention / mentionUrl 转成平台支持的 HTML 节点。

    convert_special=False:
        保存 state JSON 使用，避免 JSON 里的 mention 字符串被替换导致 JSON 失真。
    """
...
for project in projects:
    ...
    try:
        md_files, found, temp_files, step1_meta, daily_state = step1_summary_note(project)
        ...
        note_urls, note_titles = step3_generate_notes(
            ai_contents,
            project,
